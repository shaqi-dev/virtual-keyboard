...
export default class Keyboard {
  ...
  initKeys() {
    ...
    this.keyValuesDefaultEN = document.querySelectorAll(
      '.key__values_en .key__value_default',
    );
    this.keyValuesShiftEN = document.querySelectorAll(
      '.key__values_en .key__value_shift',
    );
    this.keyValuesDefaultRU = document.querySelectorAll(
      '.key__values_ru .key__value_default',
    );
    this.keyValuesShiftRU = document.querySelectorAll(
      '.key__values_ru .key__value_shift',
    );
  }
  ...
  toggleCapsLock() {
    ...
    const capsLockKey = [...this.keys].find(
      (key) => key.dataset.code === 'CapsLock',
    );
  ...
  updateKeys() {
    ...
    if (this.shift) {
      const keyValues = this.languages.en
        ? this.keyValuesShiftEN
        : this.keyValuesShiftRU;
      keyValues.forEach((key) => key.classList.add('key__value_active'));
    } else if (this.capslock) {
      const keyValuesDefault = this.languages.en
        ? this.keyValuesDefaultEN
        : this.keyValuesDefaultRU;
      const keyValuesShift = this.languages.en
        ? this.keyValuesShiftEN
        : this.keyValuesShiftRU;
      const additionalCharsRU = [
        'Backquote',
        'BracketLeft',
        'BracketRight',
        'Semicolon',
      ...
      const keyValuesFiltered = [...keyValuesShift].filter((key) => {
        const { code } = key.parentElement.parentElement.parentElement.dataset;
      ...
      keyValuesFiltered.forEach((key) => {
        key.parentElement
          .querySelector('.key__value_active')
          .classList.remove('key__value_active');
  ...
  addChar(char) {
    ...
    const setValue = (first, second) => {
      this.textarea.value = value.slice(first[0], first[1]) + char
      + value.slice(second[0], second[1]);
      pos.start += 1;
      pos.end += 1;
    };
  ...
  handleEvent = (e) => {
    switch (e.type) {
      ...
      case 'keydown': {
        const { code } = e;
        const key = [...this.keys].find((currentKey) => currentKey.dataset.code === code);
